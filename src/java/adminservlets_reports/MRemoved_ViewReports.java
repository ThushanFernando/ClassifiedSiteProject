 ...
 * @author SithuDewmi
 */
public class MRemoved_ViewReports extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet MRemoved_ViewReports</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet MRemoved_ViewReports at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (session.getAttribute("loggin_state") == "success") {
            ...
            if (request.getParameter("removeReport") != null) {
                int result = arm.updateViewState(request.getParameter("removeReport"));
                if (result == 1) {
                    String alert = "<button class=\"btn btn-green\"><i  class=\"glyphicon glyphicon-ok-sign\"></i></button><br><strong>Removed !</strong>  message id  " + request.getParameter("removeReport") + "";
                    request.setAttribute("alert", alert);
                } else {
                    String alert = "<button class=\"btn btn-red\"><i  class=\"glyphicon glyphicon-remove-circle\"></i></button><br><strong>Failed!</strong> message id " + request.getParameter("removeReport") + " Try again.";
                    request.setAttribute("alert", alert);
                }
            }
